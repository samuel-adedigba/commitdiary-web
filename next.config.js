...
const createSecurityHeaders = ({ allowEmbedding = false } = {}) => [
  {
    ...
    value: [
      ...
      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com https://sandbox-cdn.paddle.com https://cdn.paddle.com", // Allow known external stylesheets used in theme.scss and Paddle checkout
      "style-src-elem 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com https://sandbox-cdn.paddle.com https://cdn.paddle.com",
      ...
      isDevelopment
        ? "connect-src 'self' https://*.supabase.co https://*.supabase.in https://*.paddle.com https://cdnjs.cloudflare.com wss://*.supabase.co wss://*.supabase.in http://localhost:* ws://localhost:*"
        : "connect-src 'self' https://*.supabase.co https://*.supabase.in https://*.paddle.com https://cdnjs.cloudflare.com wss://*.supabase.co wss://*.supabase.in",
...
const nextConfig = {
  ...
  // Keep local chunks distinct from other Next apps served on the same origin.
  assetPrefix: isDevelopment ? '/commitdiary-dev' : undefined,
  // A production build must not overwrite the running development compiler.
