...
export const generateBadgeSVG = ({
  ...
  const bars = timeline
    .map((t, i) => {
      ...
      const y = -h;
      const opacity = t.count > 0 ? 0.7 + (t.count / maxCount) * 0.3 : 0.08;
      return `<rect x="${x}" y="${y}" width="${barWidth}" height="${Math.max(
        barWidth,
        h
      )}" fill="#3b82f6" rx="${barWidth / 2}" opacity="${opacity}" />`;
    })
    .join("");
  ...
  return `
    <svg width="100%" height="auto" viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
      ...
      <g class="header-group" transform="translate(45, 60)">
        ...
        <text y="28" class="subtitle">@${stats.username} • Live Activity Insights</text>
      </g>
      ...
      <g class="timeline-group" transform="translate(430, 130)">
        <text y="0" class="chart-title">Coding activity timeline</text>
        <g transform="translate(0, 160)">
          ${bars}
          <g transform="translate(0, 25)">
            <text class="axis-label">${timeline[0].date}</text>
            <text x="${chartWidth}" class="axis-label" text-anchor="end">${timeline[29].date}</text>
          </g>
        </g>
      </g>
      ...
      <g class="stats-group" transform="translate(45, 370)">
        ...
        <g transform="translate(740, 80)" text-anchor="end">
          <text x="-95" y="0" font-size="11" fill="#94a3b8">Powered by <tspan font-weight="700" fill="#2563eb" opacity="0.8">CommitDiary</tspan></text>
        </g>
      </g>
